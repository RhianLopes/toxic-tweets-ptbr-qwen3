...
OUTPUT = ROOT / "results" / "full" / "rag_vector_k3.csv"
...
# --- Embeddings do corpus (com cache) ---
...
if EMBEDDINGS_CACHE.exists():
    ...
    print("Computando embeddings do corpus (uma única vez)...")
    t_emb = time.time()
    corpus_embs = encoder.encode(train_texts, batch_size=256, normalize_embeddings=True, show_progress_bar=True)
    np.save(EMBEDDINGS_CACHE, corpus_embs)
    print(f"Embeddings salvos em {EMBEDDINGS_CACHE} | {(time.time()-t_emb):.1f}s")

print(f"Shape dos embeddings: {corpus_embs.shape}")
...
def retrieve(query_text: str) -> list[tuple[str, str]]:
    q_emb = encoder.encode([query_text], normalize_embeddings=True)[0]
...
# Pré-computa retrieval para todos os tweets do val antes de chamar Ollama
val_rows = list(val_df.iter_rows(named=True))
print("Computando embeddings do val e retrieval...")
t_ret = time.time()
...
all_examples = []
for q_emb in val_embs:
    sims = corpus_embs @ q_emb
    top_k_idx = np.argpartition(sims, -K)[-K:]
    top_k_idx = top_k_idx[np.argsort(sims[top_k_idx])[::-1]]
    all_examples.append([(train_texts[i], train_labels[i]) for i in top_k_idx])
