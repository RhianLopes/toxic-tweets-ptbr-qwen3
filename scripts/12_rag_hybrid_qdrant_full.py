...
OUTPUT = ROOT / "results" / "full" / "rag_hybrid_qdrant_k3.csv"
...
corpus_sparse = tfidf.fit_transform(train_texts)  # CSR matrix
...
# --- Qdrant in-memory ---
print("Indexando no Qdrant (in-memory)...")
...
BATCH = 1000
for start in range(0, len(train_texts), BATCH):
    end = min(start + BATCH, len(train_texts))
    points = []
    for i in range(start, end):
        sparse_vec = to_qdrant_sparse(corpus_sparse[i])
        points.append(
            models.PointStruct(
                id=i,
                vector={"dense": corpus_dense[i].tolist(), "sparse": sparse_vec},
                payload={"text": train_texts[i], "label": train_labels[i]},
            )
        )
    client.upsert(collection_name=COLLECTION, points=points)

print(f"Indexados {len(train_texts)} pontos no Qdrant.")
...
# Pré-computa embeddings e sparse vectors do val
...
def retrieve(idx: int) -> list[tuple[str, str]]:
    ...
    return [(p.payload["text"], p.payload["label"]) for p in results.points]
...
print("Executando retrieval híbrido para o val...")
t_ret = time.time()
all_examples = [retrieve(i) for i in range(len(val_rows))]
